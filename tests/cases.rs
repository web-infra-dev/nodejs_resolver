...
use nodejs_resolver::{AliasMap, Cache, EnforceExtension, Error, FileSystem, Options};
use nodejs_resolver::{ResolveResult, Resolver, ResolverBuilder};

#[tokio::main]
async fn should_load_description_data(
    ...
    let result = tokio::spawn(async move {
        let ResolveResult::Resource(resource) = resolver.resolve(&path, &request).await.unwrap() else {
            unreachable!();
        };
        assert_eq!(
        ...
        assert_eq!(
            *resource.description.as_ref().unwrap().data().raw().get("sideEffects").unwrap(), 
            expected_content
        );
...
#[tokio::main]
async fn should_not_found_ts_config(resolver: Arc<Resolver>, path: &Path, request: &str) {
    let path = path.to_path_buf();
    let request = request.to_string();
    let result = tokio::spawn(async move {
        assert!(matches!(resolver.resolve(&path, &request).await, Err(Error::CantFindTsConfig(_))));
    })
    .await;
    assert!(result.is_ok());
}
...
#[tokio::main]
async fn should_equal(resolver: Arc<Resolver>, path: &Path, request: &str, expected: PathBuf) {
    let path = path.to_path_buf();
    let request = request.to_string();
    let result = tokio::spawn(async move {
...
#[tokio::main]
async fn should_ignored(resolver: Arc<Resolver>, path: &Path, request: &str) {
    let path = path.to_path_buf();
    let request = request.to_string();
    let result = tokio::spawn(async move {
        match resolver.resolve(&path, &request).await {
...
#[tokio::main]
async fn should_failed(resolver: Arc<Resolver>, path: &Path, request: &str) {
    let path = path.to_path_buf();
    let request = request.to_string();
    let result = tokio::spawn(async move {
        let result = resolver.resolve(&path, &request).await;
...
fn should_overflow(resolver: Arc<Resolver>, path: &Path, request: &str) {
    let path = path.to_path_buf();
    let request = request.to_string();
    let runtime = tokio::runtime::Builder::new_multi_thread()
        .thread_stack_size(32 * 1024 * 2014)
        .build()
        .unwrap();
...
#[tokio::main]
async fn should_unexpected_json_error(
    resolver: Arc<Resolver>,
    path: &Path,
    request: &str,
...
#[tokio::main]
async fn should_unexpected_value_error(
    resolver: Arc<Resolver>,
    path: &Path,
    request: &str,
...
fn create_resolver(options: Options) -> Arc<Resolver> {
    let builder = ResolverBuilder::new(Box::new(Fs));
    Arc::new(builder.build(options))
}

#[test]
fn extensions_test() {
    let extensions_cases_path = p(vec!["extensions"]);
    let resolver = create_resolver(Options {
    ...
    should_equal(resolver.clone(), &extensions_cases_path, "./a.js", p(vec!["extensions", "a.js"]));
    should_failed(resolver.clone(), &extensions_cases_path, "./a.js/");
    should_equal(
        resolver.clone(),
    ...
    should_equal(resolver.clone(), &extensions_cases_path, "./a", p(vec!["extensions", "a.ts"]));
    should_equal(resolver.clone(), &extensions_cases_path, "./a.js", p(vec!["extensions", "a.js"]));
    should_equal(
        resolver.clone(),
        &extensions_cases_path,
    ...
    should_equal(resolver.clone(), &extensions_cases_path, ".", p(vec!["extensions", "index.js"]));
    should_equal(
        resolver.clone(),
    ...
    should_failed(resolver.clone(), &extensions_cases_path.join("index."), ".");
    should_failed(resolver.clone(), &extensions_cases_path.join("inde"), ".");
    should_equal(
        resolver.clone(),
        &extensions_cases_path,
        "m",
    ...
    should_failed(resolver.clone(), &extensions_cases_path, "./b.js");
    should_failed(resolver.clone(), &extensions_cases_path, "fs");
    should_failed(resolver.clone(), &extensions_cases_path, "./a.js/");
    should_failed(resolver.clone(), &extensions_cases_path, "m.js/");
    let resolver = create_resolver(Options {
        extensions: vec![String::from("ts"), String::from(".js")],
        ..Default::default()
    });
    ...
    should_equal(resolver.clone(), &extensions_cases_path, "./a", p(vec!["extensions", "a.js"]));
    should_equal(resolver.clone(), &extensions_cases_path, "./a.js", p(vec!["extensions", "a.js"]));
    should_equal(
        resolver.clone(),
        &extensions_cases_path,
    ...
    should_equal(resolver.clone(), &extensions_cases_path, "./a", p(vec!["extensions2", "a.js"]));
    should_equal(
        resolver.clone(),
        &extensions_cases_path,
        "./a.js",
    ...
    should_equal(resolver.clone(), &extensions_cases_path, ".", p(vec!["extensions2", "index.js"]));
    should_equal(
        resolver.clone(),
        &extensions_cases_path,
        "./index",
    ...
    should_equal(resolver.clone(), &extensions_cases_path, "./b", p(vec!["extensions2", "b"]));
    ...
    should_equal(resolver.clone(), &extensions_cases_path, "./a", p(vec!["extensions2", "a"]));
    should_equal(
        resolver.clone(),
        &extensions_cases_path,
        "./a.js",
...
fn alias_test() {
    ...
    let resolver = create_resolver(Options {
        alias: vec![
            (String::from("aliasA"), vec![AliasMap::Target(String::from("./a"))]),
            (String::from("b$"), vec![AliasMap::Target(String::from("./a/index"))]),
            (String::from("./b$"), vec![AliasMap::Target(String::from("./a/index"))]),
            (
                String::from("c$"),
                vec![AliasMap::Target(p(vec!["alias", "a", "index"]).display().to_string())],
            ),
            (
                String::from("fs"),
                vec![AliasMap::Target(
            ...
            // ---
            (String::from("./e"), vec![AliasMap::Target(String::from("./d"))]),
            (String::from("./d"), vec![AliasMap::Target(String::from("./e"))]),
            // ---
            (String::from("./f"), vec![AliasMap::Target(String::from("./g"))]),
            (String::from("./g"), vec![AliasMap::Target(String::from("./h"))]),
            (
            ...
            (String::from("recursive"), vec![AliasMap::Target(String::from("./recursive/dir"))]),
            (String::from("#"), vec![AliasMap::Target(String::from("./c/dir"))]),
            (String::from("@"), vec![AliasMap::Target(String::from("./c/dir"))]),
            (
                String::from("@start"),
            ...
            (
                ...
                vec![AliasMap::Target(String::from("@recursive/general/index.js"))],
            ),
            (
                String::from("@recursive/general"),
                vec![AliasMap::Target(String::from("@recursive/general/redirect.js"))],
            ),
            (
                String::from("@recursive"),
                vec![AliasMap::Target(String::from("@recursive/general"))],
            ),
            (String::from("./c"), vec![AliasMap::Target(String::from("./c"))]),
            (String::from("alias_with_query"), vec![AliasMap::Target(String::from("./a?q2"))]),
            (String::from("alias_with_fragment"), vec![AliasMap::Target(String::from("./a#f2"))]),
            (
    ...
    should_equal(resolver.clone(), &alias_cases_path, "./b/index", p(vec!["alias", "b", "index"]));
    should_equal(
        resolver.clone(),
        &alias_cases_path,
        "./b?aa#bb?cc",
    ...
    should_equal(resolver.clone(), &alias_cases_path, "./b", p(vec!["alias", "a", "index"]));
    should_equal(resolver.clone(), &alias_cases_path, "./b/", p(vec!["alias", "a", "index"]));
    should_equal(
        resolver.clone(),
        &alias_cases_path,
    ...
    should_equal(resolver.clone(), &alias_cases_path, "b", p(vec!["alias", "a", "index"]));
    should_equal(resolver.clone(), &alias_cases_path, "c", p(vec!["alias", "a", "index"]));
    should_equal(resolver.clone(), &alias_cases_path, "multiAlias", p(vec!["alias", "a", "index"]));
    should_failed(resolver.clone(), &alias_cases_path, "ignored/a");
    should_ignored(resolver.clone(), &alias_cases_path, "ignore/a");
    should_equal(
        resolver.clone(),
        &alias_cases_path,
        "ignore-a",
    ...
    should_overflow(resolver.clone(), &alias_cases_path, "./e");
    should_equal(resolver.clone(), &alias_cases_path, "./f", p(vec!["alias", "h", "index"]));
    should_equal(
        resolver.clone(),
        &alias_cases_path,
        "@recursive/index",
        p(vec!["alias", "node_modules", "@recursive", "general", "index.js"]),
    );
    should_equal(
        resolver.clone(),
        &alias_cases_path,
        "@recursive/general",
        p(vec!["alias", "node_modules", "@recursive", "general", "redirect.js"]),
    );
    should_equal(
        resolver.clone(),
        &alias_cases_path,
    ...
    should_equal(resolver.clone(), &alias_cases_path, "./a", p(vec!["alias", "a", "index"]));
    should_equal(resolver.clone(), &alias_cases_path, "./a/index", p(vec!["alias", "a", "index"]));
    should_equal(
        resolver.clone(),
        &alias_cases_path,
        "./a/dir",
    ...
    should_equal(resolver.clone(), &alias_cases_path, "aliasA", p(vec!["alias", "a", "index"]));
    should_equal(resolver.clone(), &alias_cases_path, "aliasA/", p(vec!["alias", "a", "index"]));
    should_equal(
        resolver.clone(),
        &alias_cases_path,
    ...
    should_equal(resolver.clone(), &alias_cases_path, "#", p(vec!["alias", "c", "dir", "index"]));
    should_equal(
        resolver.clone(),
        &alias_cases_path,
        "#/index",
    ...
    should_failed(resolver.clone(), &alias_cases_path, "@/a.js");
    should_equal(
        resolver.clone(),
        &alias_cases_path,
        "recursive",
    ...
    should_equal(resolver.clone(), &alias_cases_path, "./c", p(vec!["alias", "c", "index"]));
    should_equal(
        resolver.clone(),
        &p(vec!["in_exist_path"]),
        "@start/a",
    ...
    should_failed(resolver.clone(), Path::new("@start/a"), "");
    should_equal(resolver.clone(), &alias_cases_path, "./b/index", p(vec!["alias", "b", "index"]));
    should_equal(
        resolver.clone(),
        &alias_cases_path,
    ...
    should_equal(resolver.clone(), &alias_cases_path, "./c/index", p(vec!["alias", "c", "index"]));
    should_equal(
        resolver.clone(),
        &alias_cases_path,
        "./c/dir",
    ...
    should_ignored(resolver.clone(), &alias_cases_path, "ignore");
    // test alias ordered
    let resolver = create_resolver(Options {
        alias: vec![
            (String::from("@A/index"), vec![AliasMap::Target(String::from("./a"))]),
            (String::from("@A"), vec![AliasMap::Target(String::from("./b"))]),
        ],
        ..Default::default()
    });
    should_equal(resolver.clone(), &alias_cases_path, "@A/index", p(vec!["alias", "a", "index"]));
    ...
    should_equal(resolver.clone(), &alias_cases_path, "@A/index", p(vec!["alias", "b", "index"]));
}
...
fn fallback_test() {
    ...
    let resolver = create_resolver(Options {
        fallback: vec![
            ...
            // --
            // in-exists
            (String::from("./ee"), vec![AliasMap::Target(String::from("./dd"))]),
            (String::from("./dd"), vec![AliasMap::Target(String::from("./ee"))]),
            (String::from("./ff"), vec![AliasMap::Target(String::from("./ccc"))]),
        ],
        ..Default::default()
    });
    // maybe better is `should_overflow(resolver.clone(), &alias_cases_path, "./ee");`
    should_failed(resolver.clone(), &alias_cases_path, "./ee");
    should_failed(resolver.clone(), &alias_cases_path, "./ff");
    should_equal(resolver.clone(), &alias_cases_path, "./d", p(vec!["alias", "d", "index.js"]));
    should_equal(resolver.clone(), &alias_cases_path, "./e", p(vec!["alias", "e", "index"]));
    should_equal(resolver.clone(), &alias_cases_path, "aliasA", p(vec!["alias", "a", "index"]));
}
...
fn symlink_test() {
    ...
    let resolver = create_resolver(Options::default());
    ...
    let resolver = create_resolver(Options { symlinks: false, ..Default::default() });
...
fn simple_test() {
    ...
    let resolver = create_resolver(Options { ..Default::default() });
    should_equal(
        resolver.clone(),
        &p(vec!["in-exist-path"]),
    ...
    // as directory
    should_equal(resolver.clone(), &simple_case_path, ".", p(vec!["simple", "lib", "index.js"]));
    should_equal(resolver.clone(), &simple_case_path, "", p(vec!["simple", "lib", "index.js"]));
    should_equal(
        resolver.clone(),
...
fn pnpm_structure_test() {
    ...
    let resolver = create_resolver(Default::default());
    should_equal(
        resolver.clone(),
        &case_path.join("exports-field-a").join("lib"),
        "exports-field-aa",
        p(vec!["pnpm-structure", "node_modules", "exports-field-aa", "index.js"]),
    );
    should_failed(resolver.clone(), &case_path.join("exports-field-c"), "./b");
    should_equal(
        resolver.clone(),
        &case_path.join("exports-field-c").join("lib"),
        "exports-field-b/b",
        p(vec!["pnpm-structure", "node_modules", "exports-field-b", "index.js"]),
    );
    should_equal(
        resolver.clone(),
        &case_path.join("exports-field-c").join("lib"),
        "./b",
        p(vec!["pnpm-structure", "node_modules", "exports-field-c", "lib", "b.js"]),
    );
    should_equal(
        resolver.clone(),
        &case_path.join("exports-field-a").join("lib"),
        "exports-field-a",
        p(vec!["pnpm-structure", "node_modules", "exports-field-a", "lib", "index.js"]),
    );
    should_equal(
        resolver.clone(),
        &case_path.join("exports-field-a").join("lib"),
    ...
    should_equal(
        ...
        "module-b",
        p(vec!["pnpm-structure", "node_modules", "module-b", "index.js"]),
    );
    should_equal(
        resolver.clone(),
...
fn resolve_test() {
    ...
    let resolver = create_resolver(Options::default());
    should_equal(resolver.clone(), &fixture_path, "m1/a.js", p(vec!["node_modules", "m1", "a.js"]));
    should_equal(
        resolver.clone(),
        &fixture_path,
    ...
    should_equal(resolver.clone(), &fixture_path, "./main1.js", p(vec!["main1.js"]));
    should_equal(resolver.clone(), &fixture_path, "./main1", p(vec!["main1.js"]));
    should_equal(resolver.clone(), &fixture_path, "./main1.js?query", p(vec!["main1.js?query"]));
    should_equal(
        resolver.clone(),
        &fixture_path,
        p(vec!["./中文.js?query#fragment"]).display().to_string().as_str(),
    ...
    should_equal(resolver.clone(), &fixture_path, "./a.js", p(vec!["a.js"]));
    should_equal(resolver.clone(), &fixture_path, "./a", p(vec!["a.js"]));
    should_equal(resolver.clone(), &fixture_path, "m1/a.js", p(vec!["node_modules", "m1", "a.js"]));
    should_equal(resolver.clone(), &fixture_path, "m1/a", p(vec!["node_modules", "m1", "a.js"]));
    should_equal(
        resolver.clone(),
        &fixture_path,
        "m1/a?query",
    ...
    should_equal(resolver.clone(), &fixture_path, "./dirOrFile", p(vec!["dirOrFile.js"]));
    should_equal(resolver.clone(), &fixture_path, "./dirOrFile/", p(vec!["dirOrFile", "index.js"]));
    should_equal(
        resolver.clone(),
        &fixture_path,
        "complexm/step1",
    ...
    should_equal(resolver.clone(), &fixture_path, "m2/b.js", p(vec!["node_modules", "m2", "b.js"]));
    // edge case
    // should_equal(resolver.clone(), "./no#fragment/#/#", fixture!("no\0#fragment/\0#.\0#.js"));
    should_equal(
        resolver.clone(),
        &fixture_path,
        "./no#fragment/#/",
    ...
    should_failed(resolver.clone(), &p(vec!["no#fragment"]), "#/#");
    should_failed(resolver.clone(), &p(vec!["no#fragment", "#"]), "#");
    should_failed(resolver.clone(), &p(vec!["no#fragment", "#"]), "#.js");
    should_equal(resolver.clone(), &p(vec!["no#fragment", "#"]), "../../a", p(vec!["a.js"]));
    should_equal(
        resolver.clone(),
        &p(vec!["no#fragment", "#"]),
        "./#",
...
fn browser_filed_test() {
    ...
    should_equal(
        ...
        "browser-string",
        p(vec!["browser-module", "node_modules", "browser-string", "index.js"]),
    );
    should_equal(
        resolver.clone(),
        &browser_module_case_path.join("node_modules").join("relative").join("default"),
        "../lib/a",
        p(vec!["browser-module", "node_modules", "relative", "lib", "a.js"]),
    );

    let resolver = create_resolver(Options { browser_field: true, ..Default::default() });

    should_equal(
        resolver.clone(),
        &browser_module_case_path.join("node_modules").join("relative").join("default"),
        "../lib/a",
        p(vec!["browser-module", "node_modules", "relative", "lib", "b.js"]),
    );
    should_equal(
        resolver.clone(),
        &browser_module_case_path.join("node_modules").join("relative").join("default"),
    ...
    should_ignored(resolver.clone(), &p(vec![]), "./browser-after-main");
    should_ignored(resolver.clone(), &browser_module_case_path, ".");
    should_ignored(resolver.clone(), &browser_module_case_path, "./lib/ignore");
    should_ignored(resolver.clone(), &browser_module_case_path, "./lib/ignore.js");
    should_ignored(resolver.clone(), &browser_module_case_path, "./util.inspect");
    should_ignored(resolver.clone(), &browser_module_case_path, "./util.inspect.js");
    should_equal(
        resolver.clone(),
        &browser_module_case_path,
        "browser-string",
        p(vec!["browser-module", "node_modules", "browser-string", "target.js"]),
    );
    should_equal(
        resolver.clone(),
        &browser_module_case_path,
        "recursive-module",
        p(vec!["browser-module", "node_modules", "recursive-module", "index.js"]),
    );
    should_equal(
        resolver.clone(),
        &browser_module_case_path,
    ...
    should_failed(resolver.clone(), &browser_module_case_path, "toString");
    should_failed(resolver.clone(), &browser_module_case_path, "./toString.js");
    should_equal(
        resolver.clone(),
        &browser_module_case_path,
        "./lib/redirect",
    ...
    should_equal(
        ...
        &browser_module_case_path,
        "./lib/redirect3",
        p(vec!["browser-module", "lib", "redirect3-target", "dir", "index.js"]),
    );
    ...
    should_ignored(resolver.clone(), &lib_path, "./ignore");
    should_ignored(resolver.clone(), &lib_path, "./ignore.js");
    should_equal(
        resolver.clone(),
        &lib_path,
        "./toString",
    ...
    should_ignored(resolver.clone(), &browser_after_main_path, ".");
    should_ignored(resolver.clone(), &p(vec![]), "./browser-after-main");
    should_ignored(resolver.clone(), &browser_after_main_path, ".");
...
fn dependencies_test() {
    ...
    should_equal(
        ...
        "some-module/index",
        p(vec!["dependencies", "a", "b", "node_modules", "some-module", "index.js"]),
    );
    should_equal(
        resolver.clone(),
        &dep_case_path.join("a/b/c"), // <dep_case>/a/b/c is an in-exist path
        "module/file",
        p(vec!["dependencies", "a", "node_modules", "module", "file.js"]),
    );
    should_equal(
        resolver.clone(),
        &dep_case_path.join("a/b/c"), // <dep_case>/a/b/c is an in-exist path
        "other-module/file.js",
        p(vec!["dependencies", "node_modules", "other-module", "file.js"]),
    );
...
fn fully_specified_test() {
    ...
    let resolver = create_resolver(Options {
        alias: vec![
            (
                String::from("alias1"),
                vec![AliasMap::Target(p(vec!["full", "a", "abc"]).display().to_string())],
            ),
            (
                String::from("alias2"),
    ...
    should_failed(resolver.clone(), &full_cases_path, "package5/file");
    should_equal(
        resolver.clone(),
        &full_cases_path,
        "package5/file.js",
    ...
    should_failed(resolver.clone(), &full_cases_path, "./abc");
    should_failed(
        resolver.clone(),
        &full_cases_path,
        full_cases_path.join("abc").display().to_string().as_str(),
    );
    should_failed(resolver.clone(), &full_cases_path, "package1/file");
    should_failed(resolver.clone(), &full_cases_path, ".");
    should_failed(resolver.clone(), &full_cases_path, "./");
    should_failed(resolver.clone(), &full_cases_path, "package3/dir");
    should_failed(resolver.clone(), &full_cases_path, "package3/dir/index");
    should_failed(resolver.clone(), &full_cases_path, "package3/a");
    should_equal(
        resolver.clone(),
        &full_cases_path,
    ...
    should_equal(resolver.clone(), &full_cases_path, "./abc.js", full_cases_path.join("abc.js"));
    should_equal(
        resolver.clone(),
        &full_cases_path,
        full_cases_path.join("abc.js").display().to_string().as_str(),
    ...
    should_equal(resolver.clone(), &full_cases_path, "alias1", full_cases_path.join("abc.js"));
    should_equal(resolver.clone(), &full_cases_path, "alias2", full_cases_path.join("index.js"));
    should_equal(
        resolver.clone(),
        &full_cases_path,
        "package3",
    ...
    should_failed(resolver.clone(), &full_cases_path.join(".."), ".");
    should_equal(
        resolver.clone(),
        &full_cases_path,
        "package4/a.js",
    ...
    should_equal(resolver.clone(), &full_cases_path, "./abc.js", p(vec!["full", "a", "abc.js"]));
    should_equal(
        resolver.clone(),
        &full_cases_path,
        "package1/file.js",
    ...
    should_equal(resolver.clone(), &full_cases_path, "alias1", p(vec!["full", "a", "abc.js"]));
    should_equal(resolver.clone(), &full_cases_path, "alias2", p(vec!["full", "a", "index.js"]));
    should_equal(
        resolver.clone(),
        &full_cases_path,
        "package3",
        p(vec!["full", "a", "node_modules", "package3", "dir", "index.js"]),
    );
    should_equal(
        resolver.clone(),
        &full_cases_path,
    ...
    should_equal(resolver.clone(), &full_cases_path, ".", p(vec!["full", "a", "index.js"]));
    should_equal(resolver.clone(), &full_cases_path, "./", p(vec!["full", "a", "index.js"]));
    should_equal(
        resolver.clone(),
        &full_cases_path,
...
fn missing_test() {
    ...
    // TODO: optimize error
    // TODO: path
    should_failed(resolver.clone(), &fixture_path, "./missing-file");
    should_failed(resolver.clone(), &fixture_path, "./missing-file.js");
    should_failed(resolver.clone(), &fixture_path, "missing-module");
    should_failed(resolver.clone(), &fixture_path, "missing-module/missing-file");
    should_failed(resolver.clone(), &fixture_path, "m1/missing-file");
    should_failed(resolver.clone(), &fixture_path, "m1/");
    should_equal(resolver.clone(), &fixture_path, "m1/a", p(vec!["node_modules", "m1", "a.js"]));
}
...
fn scoped_packages_test() {
    ...
    should_equal(
        ...
        &scoped_path,
        "@scope/pack2/lib",
        p(vec!["scoped", "node_modules", "@scope", "pack2", "lib", "index.js"]),
    );
}
...
fn exports_field_test() {
    ...
    should_failed(resolver.clone(), &export_cases_path, "@exports-field/coreaaaa");
    // TODO: error stack
    should_unexpected_value_error(
        resolver.clone(),
        &export_cases_path,
    ...
    should_equal(
        ...
        p(vec!["exports-field", "node_modules", "exports-field", "package.json"]),
    );
    should_equal(
        resolver.clone(),
        &export_cases_path,
        "exports-field/dist/main.js",
        p(vec!["exports-field", "node_modules", "exports-field", "lib", "lib2", "main.js"]),
    );
    should_equal(
        resolver.clone(),
        &export_cases_path,
    ...
    should_equal(resolver.clone(), &export_cases_path, "./b", p(vec!["exports-field", "b.js"]));
    should_equal(resolver.clone(), &export_cases_path, "./a", p(vec!["exports-field", "a.js"]));
    // TODO: error stack
    should_unexpected_value_error(
        resolver.clone(),
        &export_cases_path,
    ...
    should_equal(
        ...
        p(vec!["exports-field", "node_modules", "exports-field", "lib", "main.js"]),
    );
    should_failed(resolver.clone(), &export_cases_path, "./node_modules/exports-field/dist/main");
    // TODO: error stack
    should_unexpected_value_error(
        resolver.clone(),
        &export_cases_path,
    ...
    should_equal(
        ...
        &export_cases_path,
        "./node_modules/exports-field/lib/main.js",
        p(vec!["exports-field", "node_modules", "exports-field", "lib", "browser.js"]),
    );
    ...
    should_equal(
        resolver.clone(),
        &export_cases_path,
        &p(vec!["exports-field", "node_modules", "exports-field", "main.js"]).display().to_string(),
        p(vec!["exports-field", "node_modules", "exports-field", "main.js"]),
    );
    should_unexpected_value_error(
        resolver.clone(),
        &export_cases_path,
    ...
    should_equal(
        ...
        &export_cases_path,
        "string-side-effects",
        p(vec!["exports-field", "node_modules", "string-side-effects", "index.js"]),
    );
    should_equal(
        resolver.clone(),
        &export_cases_path,
...
fn exports_filed_test_2() {
    ...
    should_equal(
        ...
        &export_cases_path2,
        "exports-field",
        p(vec!["exports-field2", "node_modules", "exports-field", "index.js"]),
    );
    should_equal(
        resolver.clone(),
        &export_cases_path2,
        "exports-field/dist/main.js",
        p(vec!["exports-field2", "node_modules", "exports-field", "lib", "lib2", "main.js"]),
    );
    should_equal(
        resolver.clone(),
        &export_cases_path2,
        "exports-field/dist/browser.js",
        p(vec!["exports-field2", "node_modules", "exports-field", "lib", "browser.js"]),
    );
    should_equal(
        resolver.clone(),
        &export_cases_path2,
        "exports-field/dist/browser.js?foo",
        p(vec!["exports-field2", "node_modules", "exports-field", "lib", "browser.js?foo"]),
    );
    should_equal(
        resolver.clone(),
        &export_cases_path2,
...
fn exports_filed_test_3() {
    ...
    should_equal(
        ...
        &p(vec!["exports-field3", "pkg1"]),
        "m1",
        p(vec!["exports-field3", "pkg1", "node_modules", "m1", "m1.js"]),
    );
}
...
fn exports_filed_test_4() {
    ...
    should_equal(
        ...
        &export_cases_path4,
        "exports-field",
        p(vec!["exports-field4", "node_modules", "exports-field", "main.js"]),
    );
    ...
    should_equal(
        ...
        &export_cases_path4,
        "exports-field",
        p(vec!["exports-field4", "node_modules", "exports-field", "index"]),
    );
}
...
fn imports_fields_test() {
    ...
    should_equal(
        ...
        &import_cases_path,
        "#a/dist/main.js",
        p(vec!["imports-field", "node_modules", "a", "lib", "lib2", "main.js"]),
    );
    should_equal(
        resolver.clone(),
        &import_cases_path,
...
fn prefer_relative_test() {
    let fixture_path = p(vec![]);
    let resolver = create_resolver(Options { prefer_relative: true, ..Default::default() });
    should_equal(resolver.clone(), &fixture_path, "main1.js", p(vec!["main1.js"]));
    should_equal(resolver.clone(), &fixture_path, "m1/a.js", p(vec!["node_modules", "m1", "a.js"]));
}
...
//     let resolver = create_resolver(Options { ..Default::default() });
//     should_equal(resolver.clone(), &fixture_path, ".", p(vec!["cache-fs", "src", "index.js"]));

//     write(fixture_path.join("package.json"), "{\"main\": \"./src/module.js\"}")
//         .expect("write failed");

//     resolver.clear_entries();
//     sleep(Duration::from_secs(1));

//     should_equal(resolver.clone(), &fixture_path, ".", p(vec!["cache-fs", "src", "module.js"]));

//     write(fixture_path.join("package.json"), "{\"main\": \"./src/index.js\"}")
//         .expect("write failed");
...
//     let resolver = create_resolver(Options { ..Default::default() });
//     should_equal(resolver.clone(), &fixture_path, "./index", p(vec!["cache-fs2", "index.js"]));
//     rename(fixture_path.join("index.js"), fixture_path.join("temp.js")).expect("rename failed");
//     sleep(Duration::from_secs(1));
//     should_equal(resolver.clone(), &fixture_path, "./index", p(vec!["cache-fs2", "index.js"]));
//     resolver.clear_entries();
//     should_failed(resolver.clone(), &fixture_path, "./index");
...
fn tsconfig_paths_test() {
    ...
    should_equal(resolver.clone(), &tsconfig_path, "", p(vec!["tsconfig-paths", "index.ts"]));
    should_equal(resolver.clone(), &tsconfig_path, "?a", p(vec!["tsconfig-paths", "index.ts?a"]));
    should_equal(
        resolver.clone(),
        &tsconfig_path,
        "actual/test",
    ...
    should_failed(resolver.clone(), &tsconfig_path, "te*t3/foo");
    should_equal(
        resolver.clone(),
        &tsconfig_path,
        "test3/foo",
...
fn tsconfig_paths_nested() {
    ...
    should_equal(
        ...
        &tsconfig_path,
        "test2/foo",
        p(vec!["tsconfig-paths-nested", "nested", "test2-success", "foo.ts"]),
    );
    should_failed(resolver.clone(), &tsconfig_path, "te*t3/foo");
    should_equal(
        resolver.clone(),
    ...
    should_equal(
        ...
        &tsconfig_path,
        "test4/foo",
        p(vec!["tsconfig-paths-nested", "nested", "test4-first", "foo.ts"]),
    );
    should_equal(
        resolver.clone(),
        &tsconfig_path,
        "test5/foo",
        p(vec!["tsconfig-paths-nested", "nested", "test5-second", "foo.ts"]),
    );
    should_equal(
        resolver.clone(),
        &tsconfig_path,
...
fn tsconfig_paths_without_base_url_test() {
    ...
    should_failed(resolver.clone(), &case_path, "should-not-be-imported");
    should_equal(resolver.clone(), &case_path, "alias/a", case_path.join("src/a.ts"));
    should_equal(
        resolver.clone(),
        &case_path,
...
fn tsconfig_paths_missing_base_url() {
    ...
    should_failed(resolver.clone(), &case_path, "#/test");
}
...
fn tsconfig_inexist() {
    ...
    should_not_found_ts_config(resolver, &p(vec![]), "./a.js");
}
...
fn load_description_data() {
    ...
    should_load_description_data(
        resolver.clone(),
        &case_path,
        "@scope/import-require",
        p(vec!["exports-field", "node_modules", "@scope", "import-require"]),
    ...
    should_load_description_data(
        ...
        &case_path,
        "string-side-effects",
        p(vec!["exports-field", "node_modules", "string-side-effects"]),
...
fn shared_cache_test2() {
    ...
    should_ignored(resolver.clone(), &case_path, "./lib/ignore.js");

    let resolver =
        create_resolver(Options { external_cache: Some(cache.clone()), ..Default::default() });
    should_equal(
        resolver.clone(),
        &case_path,
        "./lib/ignore.js",
...
fn empty_test() {
    ...
    let resolver = create_resolver(Options::default());
    should_failed(resolver.clone(), &case_path, ".");
    should_failed(resolver.clone(), &p(vec![]), "./empty");
}
...
fn browser_it_self() {
    ...
    //     &case_path,
    //     "b.js/package.json",
    //     p(vec!["browser-to-self", "node_modules", "b.js", "package.json"]),
    // );
    should_overflow(resolver.clone(), &case_path, "c.js");
    // let resolver = create_resolver(Options {
    //     browser_field: true,
    ...
    //     p(vec!["browser-to-self", "node_modules", "c.js", "c.js"]),
    // );
    // let resolver = create_resolver(Options { browser_field: false, ..Default::default() });
    // should_equal(
    //     resolver.clone(),
    //     &case_path,
    //     "a.js",
    //     p(vec!["browser-to-self", "node_modules", "a.js", "a.js"]),
    // );
    // should_failed(resolver.clone(), &case_path, "c.js");
}
...
fn resolve_to_context_test() {
    let resolver = create_resolver(Options { resolve_to_context: true, ..Default::default() });
    should_equal(resolver.clone(), &p(vec![]), "./", p(vec![]));
    should_equal(resolver.clone(), &p(vec![]), "./dirOrFile", p(vec!["dirOrFile"]));
    should_equal(
        resolver.clone(),
        &p(vec![]),
    ...
    should_equal(resolver.clone(), &p(vec![]), "./m.js", p(vec!["m.js"]));
    should_equal(resolver.clone(), &p(vec![]), "./main-field", p(vec!["main-field"]));
    should_equal(
        resolver.clone(),
...
fn resolve_modules_test() {
    ...
    should_equal(resolver.clone(), &p(vec![]), "a", p(vec!["alias", "a", "index"]));
    let resolver = create_resolver(Options {
    ...
    should_failed(resolver.clone(), &p(vec![]), "recursive-module");
}
...
fn extension_alias() {
    let resolver = create_resolver(Options {
        ...
        extension_alias: vec![
            (".js".to_string(), vec![".ts".to_string(), ".js".to_string()]),
    ...
    should_failed(resolver.clone(), &fixture, "a");
    should_failed(resolver.clone(), &p(vec!["extension-alias", "node_modules"]), "./a.js");
    should_equal(
        resolver.clone(),
    ...
    should_equal(resolver.clone(), &fixture, "./index", p(vec!["extension-alias", "index.js"]));
    should_equal(resolver.clone(), &fixture, "./index.js", p(vec!["extension-alias", "index.ts"]));
    should_equal(
        resolver.clone(),
        &fixture,
        "./dir/index.js",
    ...
    should_failed(resolver.clone(), &fixture, "./index.mjs");
    ...
    should_failed(resolver.clone(), &p(vec!["extension-alias", "node_modules"]), "./a.js/index");
}
